...
class MembershipController extends Controller
{
    ...
    {   
        ...
        /**
         * here will be generate the solidary bonuses 
         */

        // $user
        $bebe = new User('bebe');
        // User::->getMembershipStatus(){
        $bebe->address= '0x80A7f6E4c9BaB2B345876D031c949eFbDaskkdi9';
        ...
        $bebe; //User::findOrFail($user_from->id);
        
        $bonusSolidarioPolicy = new BonusSolidarioPolicy($rulesPolicySolidary, $bebe, $price);
        $bonusSolidarioPolicy->baseprice = 1000;

        $bonus = new BonusSolidary($bonusSolidarioPolicy);
        $bebe->directs = [
            "amalia", "bebe", "tita", "ochenta"
        ];
        // foreach($user->getGeferralDirects())
        $bonus->addPartner($bebe);
        $bonus->addPartner($bebe);
        $bonus->addPartner($bebe);
        $solidarybonus = new SolidaryBonus();
        $bonus->addPartner($bebe);
        
        $membership->bonus_solidary_fee;

        $solidary_bonus = round($membership_price * $membership->bonus_solidary_fee / 100, 2);
        print_r($bonus->redimir());
        /*
        ...
        $Twallet->trans_id = rand();
        $Twallet->time = Carbon::now();
        $Twallet->description = 'Desembolso a ' . $user_to->username . ' Bono Solidario '. $trans_description . ' ' . $user->username;
        $Twallet->amount = $quick_bonus * -1;
        $Twallet->type = 16;
        $Twallet->charge = 0;
        $Twallet->model_ref = $model;
